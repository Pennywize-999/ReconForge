    ...
    def parse(cls, file_path: str) -> Tuple[List[Host], List[Finding], List[str]]:
        ...
        if subject_match:
            ...
            finding = Finding(
                ...
                source_type="TLS Analysis",
                evidence=[Evidence(source_file=filename, source_type="TLS Analysis", content=subject_match.group(0))],
            )
        ...
        if "Verify return code: 10 (certificate has expired)" in content:
            finding = Finding(
                ...
                source_file=filename,
                source_type="TLS Analysis",
                evidence=[Evidence(source_file=filename, source_type="TLS Analysis", content="Verify return code: 10")],
            )
            host.findings.append(finding)
