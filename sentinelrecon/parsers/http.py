...
class HTTPParser(BaseParser):
    ...
    def parse(cls, file_path: str) -> Tuple[List[Host], List[Finding], List[str]]:
        ...
        if server_match:
            ...
            host.findings.append(
                Finding(
                    ...
                    source_type="HTTP Response Analysis",
                    evidence=[Evidence(source_file=filename, source_type="HTTP Response Analysis", content=content[:12000])],
                )
            )
        ...
        if endpoint is not None and not any(
            e.url == endpoint.url and e.status_code == endpoint.status_code for e in host.web_endpoints
        ):
