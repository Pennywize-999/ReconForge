...
from sentinelrecon.web.waf.analyzer import WAFAnalyzer
...
class Analyzer:
    ...
    def analyze_directory(self, directory_path: str) -> Target:
        ...
        for root, _, files in os.walk(directory_path):
            for file in files:
                if file in self.INTERNAL_FILES or (file.startswith("sentinelrecon_") or file.startswith("reconforge_")) and file.endswith("_wordlist.txt"):
                    continue
        ...
        self._run_vulnerability_assessment(target)
        return target
    ...
    def analyze_file(self, file_path: str, target: Optional[Target] = None) -> Target:
        ...
        if not os.path.basename(file_path).endswith(self.INTERNAL_LOG_SUFFIXES):
            self._extract_unclassified_intelligence(file_path, target)

        self._run_waf_analysis(target)
        self._run_vulnerability_assessment(target)
        return target

    def _run_vulnerability_assessment(self, target: Target) -> None:
        try:
            engine = VulnerabilityEngine()
            engine.assess_target(target)
        except Exception as exc:
            target.evidence.append(Evidence("vulnerability_engine", "VulnerabilityEngine", f"Error during vulnerability assessment: {exc}"))

    def _extract_unclassified_intelligence(self, file_path: str, target: Target):
        base_name = os.path.basename(file_path)
        if base_name in self.INTERNAL_FILES or any(base_name.endswith(sfx) for sfx in self.INTERNAL_LOG_SUFFIXES):
            return
    ...
    def _run_waf_analysis(self, target: Target):
        ...
        for host in target.hosts.values():
            if host.web_endpoints or any(p.service and "http" in p.service.name.lower() for p in host.ports):
                host.waf_analysis = analyzer.analyze(host, target.evidence)

    def _merge_host(self, target: Target, new_host: Host):
        existing_host = None
        if new_host.ip and new_host.ip != "unknown" and new_host.ip in target.hosts:
    ...
    def _merge_host_data(self, existing_host: Host, new_host: Host):
        ...
        for finding in new_host.findings:
            ...
            if matched:
                matched.evidence.extend(finding.evidence)
            else:
